...
from intellipy.Sockets.RS232 import RS232  # socket type


class ConnectToIntellivueRS232(ConnectionProtocol.ConnectionProtocol):
    ...
    def __init__(self, selectedDataTypes, SaveProtocol, device):
    ...
    def connect(self):
        """Connect to vitals montior via RS232"""
    ...
    def initiateAssociation(self):
        ...
        Send AssociationRequest, read in AssociationResponse
        and MDSCreateEvent, send MDSCreateEventResult
        ...
        # 2) Read in Association Response, MDS Create Event, send MDS Create Event Result
        no_association = True
        assoc_aborted = True
        while no_association and self.isConnecting:
            message1 = self.socket.receive()
            message_type1 = self.Intellivue.getMessageType(message1)
            print("Received " + message_type1 + ".")

            count = 0
            # If message1 isn't an Association Response...
            while assoc_aborted and self.isConnecting:
                if message_type1 != "AssociationResponse":
                    # If data still coming in, abort/release
                    if (
                        message_type1 == "MDSExtendedPollActionResult"
                        or message_type1 == "LinkedMDSExtendedPollActionResult"
                        or message_type1 == "Unknown"
                    ):
                        # Every 50 messages (~10 s) try resending association abort and release request
                        if count % 50 == 0:
                            # abort/release
                            self.socket.send(self.AssociationAbort)
                            self.socket.send(self.ReleaseRequest)
                            print("Sent Association Abort and Release Request...")
                            count = 0
                        ...
                        count += 1
                    # If aborted or released, re-initiate connection
                    elif (
                        message_type1 == "AssociationAbort"
                        or message_type1 == "ReleaseRequest"
                    ):
                        # Reopen port
                        self.socket.close()
                        self.socket = RS232(self.device)
            ...
            # Should receive Association Response and MDSCreateEvent
            if (
            ...
            # If not, then re-send Association Request
            else:
    ...
    def setPriorityLists(self):
        ...
        while no_priority_list:
            ...
            if message_type == "MDSGetPriorityListResult":
                print(self.Intellivue.readData(message))
    ...
    def endConnection(self):
        ...
        while not_refused:
            ...
            if (
                ...
                or message_type == "AssociationAbort"
                or message_type == "Unknown"
            ):
